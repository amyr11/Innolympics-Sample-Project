...
class FirestoreService {
  ...
  FirestoreService() {
    _goalsColRef = _db.collection(goalCollection).doc(uid).collection(goalCollection);
  }
  ...
  Stream<Goal> streamGoal(String id) {
    return _goalsColRef.doc(id).snapshots().map((snapshot) => Goal.fromFirestore(snapshot));
  }
}
